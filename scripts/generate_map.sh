...
ROOT_PATH=$(sudo -u "$CURRENT_DIR_USER" git -C "$(dirname "$(readlink -f "$0")")" rev-parse --show-toplevel)
SERVICE_NAME=$(basename "$ROOT_PATH")
REPOSITORY_OWNER=$(stat -c '%U' "$ROOT_PATH")
...
# logic: Use ROOT_PATH as default if no argument is provided
TARGET_DIR="${1:-$ROOT_PATH}"
...
if [ "$PROJECT_ROOT" == "$ROOT_PATH" ]; then
    FORCE_INCLUDE=("app")
fi
...
if [[ -n "$NESTED_MAPS" ]]; then
    ...
    while read -r map_path; do
        rel_map_path=${map_path#$ROOT_PATH/}
